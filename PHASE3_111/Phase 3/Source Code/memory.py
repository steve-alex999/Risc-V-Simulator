...
def memory_access(m_index,m_value,m_type,m_read):
    if m_read == 0:     # store
        if m_index > -1:
            if 15 <= executed_information.executed_info[-2][0] <= 18 and executed_information.executed_info[-2][1] == -executed_information.executed_info[-1][1]:
                m_value = executed_information.executed_info[-1][6]             ## M-M fwding
                executed_information.data_hazards += 1
            elif 15<= executed_information.executed_info[-3][0] <=18 and executed_information.executed_info[-3][1] == -executed_information.executed_info[-1][1]:
                m_value = executed_information.executed_info[-2][6]             ## M-M fwding
                executed_information.data_hazards += 1

            if m_type == 0:     write_byte_to_mem(m_index, m_value)
            elif m_type == 1:   write_hword_to_mem(m_index,m_value)
            elif m_type == 2:   write_word_to_mem(m_index,m_value)
            elif m_type == 3:   write_dword_to_mem(m_index,m_value)
    elif m_read == 1:
        if m_type == 0:         r_value = read_byte_from_mem(m_index)
        elif m_type == 1:       r_value = read_hword_from_mem(m_index)
        elif m_type == 2:       r_value = read_word_from_mem(m_index)
        elif m_type == 3:       r_value = read_dword_from_mem(m_index)
        return r_value
